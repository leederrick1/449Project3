...
class SessionStore:
    ...
    def set_key(self, key, value):
        ...
        pair= {key: value}
        requests.post("http://localhost:5100/", json= pair)
        return
        # raise NotImplementedError()
    #wants the value returned
    def get_key(self, key):
        self.logger.debug("get_key('%s')", key)
        r = requests.get("http://localhost:5100/" + str(key)).json()
        return r[key]

    def delete_key(self, key):
        self.logger.debug("delete_key('%s')", key)
        requests.delete("http://localhost:5100/" + str(key))
        return 
